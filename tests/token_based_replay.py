...
parameters1 = {'cache':{'bst':False,'activity':False},'token_flooding':{'handle':True,'method':'boundness'}}

'''for path in [path_p2p,path_o2c]:
    ocel = ocel_import_factory.apply(path,parameters={"debug": False})
    cached_parameters = {'BST':False,'activity':True}
    ocpn = ocpn_discovery_factory.apply(ocel, parameters={"debug": False})
    time0= time.time() 
    result = token_based_replay.apply(ocel,ocpn,method='object_centric',parameters=parameters1)
    time1= time.time()
